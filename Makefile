...
# HYUNDAI_COMMANDO_SOURCE_FILES = plugin.yml $(shell find src resources -type f)
HYUNDAI_COMMANDO_SOURCE_FILES = plugin.yml $(shell find src -type f)
# HYUNDAI_COMMANDO_VIRIONS = dev/await-generator.phar dev/await-std.phar dev/libasynql.phar dev/rwlock.phar
HYUNDAI_COMMANDO_VIRIONS = dev/await-generator.phar dev/await-std.phar dev/libMarshal.phar dev/Commando.phar

# .PHONY: all phpstan fmt debug/suite-mysql suitetest $(SUITE_TESTS)
.PHONY: all phpstan fmt suitetest $(SUITE_TESTS)

default: phpstan dev/HyundaiCommando.phar
...
dev/HyundaiCommando.phar: $(HYUNDAI_COMMANDO_SOURCE_FILES) dev/ConsoleScript.php $(HYUNDAI_COMMANDO_VIRIONS)
# 	$(PHP) dev/ConsoleScript.php --make plugin.yml,src,resources --out $@
	$(PHP) dev/ConsoleScript.php --make plugin.yml,src --out $@

	for file in $(HYUNDAI_COMMANDO_VIRIONS); do $(PHP) $$file $@ keopiwauyu\\HyundaiCommando\\Virions\\$$(tr -dc A-Za-z </dev/urandom | head -c 8)\\ ; done

# src/SOFe/Capital/Database/RawQueries.php: dev/libasynql.phar resources/mysql/* resources/sqlite/*
# 	$(PHP) dev/libasynql.phar fx src/ keopiwauyu\\HyundaiCommando\\Database\\RawQueries --struct 'final class' --spaces 4 --sql resources --prefix capital
...
SKIP_MYSQL = true # no mysql in hyundai commando

# $(SUITE_TESTS): dev/HyundaiCommando.phar dev/FakePlayer.phar dev/InfoAPI.phar dev/SuiteTester.phar
$(SUITE_TESTS): dev/HyundaiCommando.phar dev/FakePlayer.phar dev/SuiteTester.phar
	$(eval CONTAINER_PREFIX := hyundai-commando-suite-$(shell basename $@))
...
# 	docker cp dev/InfoAPI.phar $(CONTAINER_PREFIX)-pocketmine:/plugins/InfoAPI.phar
	...
	docker cp dev/HyundaiCommando.phar $(CONTAINER_PREFIX)-pocketmine:/plugins/HyundaiCommando.phar
...
# debug/suite-mysql:
# 	docker exec -it hyundai-commando-suite-mysql-mysql bash -c 'mysql -u $$MYSQL_USER -p$$MYSQL_PASSWORD $$MYSQL_DATABASE'
